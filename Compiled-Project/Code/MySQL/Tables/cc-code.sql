-- Drop existing cc table if it exists
...
CREATE TABLE cc (
    ...
    `when` DATETIME,
    `who` INT(11),
    UNIQUE KEY `unique_row` (`id`, `when`, `who`)
);
...
-- Remove duplicates in the temporary table based on the unique key
...
SELECT * 
INTO OUTFILE '/var/lib/mysql/Final_Project/Final_CSV-files/cc_cleaned.csv'
FIELDS TERMINATED BY ','
LINES TERMINATED BY '\n'
FROM cc;
...
-- Load data from CSV file into the temporary table
LOAD DATA INFILE '/var/lib/mysql/Final_Project/Final_CSV-files/cc_cleaned.csv'
INTO TABLE temp_cc
FIELDS TERMINATED BY ','
LINES TERMINATED BY '\n'
IGNORE 1 LINES
