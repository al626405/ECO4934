-- Drop existing priority table (only if you need to recreate it)
DROP TABLE IF EXISTS priority;

-- Create the priority table with the desired structure
...
DELETE FROM temp_priority
WHERE id IS NULL
   OR `what` IS NULL
   OR `when` IS NULL
   OR `who` IS NULL;

-- Insert data from temporary table into the priority table
INSERT INTO priority (id, `what`, `when`, `who`)
SELECT id,
       CASE
           WHEN `what` NOT IN ('P1', 'P2', 'P3', 'P4', 'P5', 'NONE') THEN 'NONE'
           ELSE `what`
       END as `what`,
       `when`,
       `who`
...
DROP TEMPORARY TABLE temp_priority;
