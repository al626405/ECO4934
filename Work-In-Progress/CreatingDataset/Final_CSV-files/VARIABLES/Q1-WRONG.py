...
merged_df = merged_df.drop(columns=['id_bug', 'id_resolution'])

# Filter out rows where 'assigned_to' is 'None'
filtered_df = merged_df[merged_df['assigned_to'] != 'None']

# Select relevant columns
final_df = filtered_df[['id_assigned', 'report_id', 'assigned_who', 'assigned_to', 'assigned_when', 'bug_status', 'bug_who', 'bug_when', 'resolution', 'res_who', 'res_when']]

# Save the resulting DataFrame to a new CSV file
final_df.to_csv("/home/Alexis/Database/Python/calculatedstatistics/Q1_dataset.csv", index=False)
